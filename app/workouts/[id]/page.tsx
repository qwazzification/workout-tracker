...
function extractLocalTime(isoStr: string | null): string {
  if (!isoStr) return ''
...
function computeWorkoutDuration(date: string, startTime: string, endTime: string): number | null {
  if (!startTime || !endTime) return null
  const start = new Date(`${date}T${startTime}:00`).getTime()
  let end = new Date(`${date}T${endTime}:00`).getTime()
  if (end < start) end += 24 * 60 * 60 * 1000 // crossed midnight
...
export default function WorkoutDetailPage() {
  ...
  const [editStartedAt, setEditStartedAt] = useState('')   // "HH:MM"
  const [editEndedAt, setEditEndedAt] = useState('')       // "HH:MM"
  ...
  async function loadData() {
    ...
    setEditStartedAt(extractLocalTime((log as unknown as { started_at: string | null }).started_at))
    setEditEndedAt(extractLocalTime((log as unknown as { ended_at: string | null }).ended_at))
  ...
  async function saveEdits() {
    ...
    const { error: logError } = await supabase
      ...
      .update({
        ...
        started_at: buildTimestamp(date, editStartedAt),
        ended_at: buildTimestamp(date, editEndedAt),
        duration_seconds: computeWorkoutDuration(date, editStartedAt, editEndedAt),
      })
      .eq('id', id)
              ...
              <div>
                ...
                {(workout.started_at || workout.ended_at) ? (
                  <div className="text-sm text-gray-500 dark:text-gray-400 mt-0.5">
                    ...
                    {' – '}
                    {workout.ended_at ? format(new Date(workout.ended_at), 'h:mm a') : '?'}
      ...
      {mode === 'edit' && (
        <>
          <div className="bg-white dark:bg-gray-800 rounded-xl p-4 shadow-sm border border-gray-100 dark:border-gray-700 mb-4 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Start & end time (optional)</label>
              ...
              {editStartedAt && editEndedAt && (() => {
                const d = computeWorkoutDuration(date, editStartedAt, editEndedAt)
                return d ? (
